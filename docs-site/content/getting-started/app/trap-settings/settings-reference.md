...
| **EDC Backoff (ms)** | 30 to 130 ms (10 ms steps) | 50 ms | Reverse timing during the EDC re-seat. Start calibration at 40 ms with a fully charged battery. Raise it if the door does not retract enough; lower it if the door reverses so far that it lifts. The door must finish under the fixed bar every time. Needs firmware v2.3.2-1010+. |

---
description: >-
  Specs, connectivity, LED patterns, and battery details. Look here when you
  need a number or a status light explained.
---

# Reference

Use these pages when you need a specification or an explanation of a status light.

{% content-ref url="hardware-features.md" %}
[Hardware Features](hardware-features.md)
{% endcontent-ref %}

{% content-ref url="technical-specifications.md" %}
[Technical Specifications](technical-specifications.md)
{% endcontent-ref %}

{% content-ref url="connectivity-and-coverage.md" %}
[Connectivity & Coverage](connectivity-and-coverage.md)
{% endcontent-ref %}

{% content-ref url="led-guide.md" %}
[LED Guide](led-guide.md)
{% endcontent-ref %}

{% content-ref url="battery-overview.md" %}
[Battery Overview](battery-overview.md)
{% endcontent-ref %}

{% content-ref url="../faqs/power-modes.md" %}
[Power Modes](../faqs/power-modes.md)
{% endcontent-ref %}

{% content-ref url="accessory-port.md" %}
[Accessory Port](accessory-port.md)
{% endcontent-ref %}

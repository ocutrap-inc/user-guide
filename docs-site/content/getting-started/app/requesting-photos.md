---
description: >-
  Ask your trap for a fresh photo at any time. Covers full-quality and quick
  image requests, the camera view, and browsing past images.
---

# Requesting photos

You can request a fresh photo of the inside of your trap at any time. Open the trap's detail screen. The most recent photo appears as a large image (the **camera view**) with the trap's controls beside it. There's no separate step to open the camera view.

## Request a photo

To ask the trap for a new photo, use **Request image**. The trap captures a standard, full-quality photo and sends it to the app. Use this option to confirm what's inside or check image clarity.

In the mobile app, you can also choose **Request quick image** for a faster, lower-resolution photo. Use it when you need a quick check without full detail.

<figure><img src="../../.gitbook/assets/IMG_2083.png" alt="" width="375"><figcaption></figcaption></figure>

After you request a photo, an **Image incoming…** indicator appears on the camera view while the trap captures and uploads the new image. It clears on its own once the fresh photo arrives.

## Browse past images

From the camera view you can:

* Tap the image to open the gallery
* Use the **Images** tab to see previously captured photos

If images look too dark, washed out, or blurry, see [Camera](../../faqs/camera.md) for light-switching settings and fixes.

## See also

* [Trap Control](trap-control.md): all the commands you can send your trap
* [When Your Trap Takes Photos](image-cadence.md): automatic capture timing
* [Camera](../../faqs/camera.md): image quality and night-vision settings
